...
export const PopularItemsGallery: React.FC = () => {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-72 bg-gray-50 animate-pulse rounded-2xl border border-gray-100"
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center space-y-2">
        <p className="text-brand-neutral font-medium">
          No popular items to display right now.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          {isFetching && (
            <Loader2
              size={14}
              className="animate-spin text-brand-secondary ml-2"
            />
          )}
        </div>
      ...
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 transition-opacity duration-300 ${isFetching ? "opacity-40" : "opacity-100"}`}>
